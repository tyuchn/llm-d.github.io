...
In this post, we share our production evaluation of **hierarchical KV cache offloading with SGLang and llm-d** on Google Kubernetes Engine (GKE) using NVIDIA B200 GPUs (`a4-highgpu-8g`). We deploy a 3-tier memory hierarchy orchestrated by SGLang's Hierarchical Cache (**HiCache**) engine and fronted by [`llm-d-router`](https://github.com/llm-d/llm-d-router):
